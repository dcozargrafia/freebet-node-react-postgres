// components/BookmakersTable.jsx
...
const BookmakersTable = ({ data, onDataChange }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`${URL}/bookmakers/${deleteDialog.row.id}`, {
        method: 'DELETE',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Error deleting bookmaker');
      }
  ...
  const renderCell = (row, columnId) => {
    switch (columnId) {
      ...
      case 'type':
        return (row.type === 'regular') ? 'Normal' : 'Exchange';
      default:
        return row[columnId];
    }
  };
